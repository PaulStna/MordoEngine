...
// camera exists.
//
// It is also the one that aims. Every frame it casts down the view and remembers
// what it found, so the crosshair has something to report before anything is
// pressed, and so using something is just calling Interact on what was already
// there.
//
...
public:
	...
	// What the crosshair is on, or null. Refreshed every frame, so it is only
	// good for the frame it was read in: an actor destroyed since would leave
	// this dangling if it were kept.
