...
EditorScene::EditorScene(EngineContext& context)
	: m_Context(context)
{
	World& world = m_Context.GetWorld();
	m_CameraController = std::make_unique<EditorCameraController>(world.GetCamera());
	m_EditorSystem = std::make_unique<EditorSystem>(m_Context.Shaders().Get("terrainSelector"));
...
void EditorScene::Render()
{
	...
	Shader& terrainShader = m_Context.Shaders().Get("terrain");
