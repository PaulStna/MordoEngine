...
- Split Model into the asset and the instance. Model now holds only what every copy shares, the GPU buffers, the pieces and a new Rig with the skeleton and the clips, while a new Body carries the Animator that belongs to one actor. ResourceLibrary keeps a single Model per file, so without this two foxes shared a transform and a pose and could neither stand apart nor run different clips.
- Moved the transform out of Model as well, leaving the actor's as the only one. Actor now holds a Body instead of a Model pointer, so SyncToModel is gone along with the copy it made every frame.
- Made ActorSystem drive the drawing and advance the animations, and left ModelSystem owning nothing: BeginPass uploads the uniforms the whole pass shares once instead of once per model, then Render puts a body and its transform together. A destroyed actor now takes its body with it, so it no longer leaves its model standing where it was.
- Added Animator::SetTime so two animals on the same clip can start at different points in it. Play always restarts from zero, and without an offset they move in lockstep and read as one animal drawn twice.
- Made LoadModels skip a model whose file is missing instead of throwing out of the constructor, so the scene still comes up without it.

## 2026-08-01
- Gave every actor an interaction path: a virtual Interact that reports whether anything happened, and a flag kept beside it so a query can skip whatever would only refuse. PlayerActor casts a ray down the camera every frame and remembers what it found, so a prompt can be shown before anything is pressed. Input reaches it through ActorContext as plain data, which keeps GLFW out of the actor layer.
- Added collision under src/Physics, on a new Bounds and Ray in Core/Math. Collider is abstract with two methods, a local box for the broad phase and an exact local space raycast for the narrow one, so a shape that does not exist yet is one class and nothing that queries it changes. Box and Sphere implement it, and a layer mask filters queries.
- Made ModelLoader measure the model's extent while it loads, putting the node transform back in for the one submesh case that kept it out, so Actor::SetModel fits a box collider off the geometry without anyone asking for one. Hits bring the ray into local space rather than dragging the box out into the world, which stays exact under rotation and scale and still hands back a world distance.
- Added ActorSystem::Raycast, for the nearest actor along a ray, and OverlapBounds, for box against box. Both filter by layer first. A linear scan for now, which is the piece a broad phase structure replaces later without any caller noticing.
- Made point lights switchable. A light that is off keeps its colours and its place and only stops reaching the shaders, LightActor flips it on interact, and ApplyUniforms now packs the live ones into consecutive slots. That last part also fixes numPointLights being uploaded as the full count while the loop stopped at 16, which had the shader reading uniforms nobody wrote.
- Added a way to see the collision: F3 draws every collider, picks out the one under the crosshair and marks where the aim ray landed. Two files under Renderer/Debug hold all of it, a line batcher that knows nothing about what it draws and the one class that knows collision debugging exists, so deleting the pair removes the feature and nothing in Actor, Collider or ActorSystem points back at it. It draws the box through the actor's matrix rather than the refitted world one, since showing a volume looser than the one a ray is tested against would be debugging a lie, and it runs at the end of World::Render rather than in RenderOpaque, which the water passes call three times a frame.
- Added ActorSystem::ForEachActor and PlayerActor::GetAimDirection, the only two things outside those files the view needed. Both are read only, and neither is about debugging: one is a walk over the list nobody owned a way to do, the other exposes a direction that was already being computed.